...
const normalizeLocalRecord = (moduleKey, form) => ({
  ...
  owner: form.owner || "Demo User",
  order: form.order || "",
});

const fetchBootstrap = async () => {
  const apiResponse = await fetch("/api/bootstrap").catch(() => null);
  if (apiResponse?.ok) return apiResponse.json();

  const staticResponse = await fetch("/api/bootstrap.json").catch(() => null);
  if (staticResponse?.ok) return staticResponse.json();

  throw new Error("Unable to load accounting demo data");
};
...
function App() {
  ...
  useEffect(() => {
    fetchBootstrap()
      ...
      .catch(() => {
        setToast("Unable to load demo data");
      });
  }, []);
  ...
  const saveRecord = async (moduleKey, form) => {
    ...
    try {
      ...
      payload = { record: normalizeLocalRecord(moduleKey, form) };
    }
    setRecords((current) => {
  ...
  const deleteRecord = async (moduleKey, id) => {
    await fetch(`/api/records/${moduleKey}/${id}`, { method: "DELETE" }).catch(() => null);
    setRecords((current) => ({
      ...current,
